...
const Login = () => {
  ...
  const fillDemoCredentials = () => {
    setValue('email', 'farmer@test.com');
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const success = await login('google.user@gmail.com', 'google_auth_token', true);
      if (success) {
        toast({
          title: "Google Sign-In Successful!",
          description: "Welcome to AgroConnect",
        });
        navigate('/dashboard');
      }
    } catch (error) {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <motion.div variants={itemVariants} className="text-center">
              <button
                ...
                className="text-xs text-primary hover:text-primary/80 underline mb-2 transition-colors"
              >
